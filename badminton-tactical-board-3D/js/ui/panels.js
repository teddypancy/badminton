...
export function updateParamPanel() {
  ...
  const cols = playerEntries.length >= 3 ? 2 : playerEntries.length;
  ...
  html += `
    ...
    ${pressHighWarn
      ...
      : `<div style="font-size:10px; color:#78909c; margin-top:-2px; margin-bottom:6px; padding-left:2px;">
          ${isFirstShot ? '🔒 第1拍固定低位發球 (1.15m)' : '擊球高度：' + (shot.hitPoint && shot.hitPoint.y !== undefined ? shot.hitPoint.y.toFixed(2) + 'm' : '無攔截')}
        </div>`}
  `;
