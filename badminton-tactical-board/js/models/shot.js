...
export function matchReceiverSpeed(shot, shots) {
  ...
  if (nextShot && !nextShot.isSetup && !nextShot.pendingTo && startPos) {
    ...
    if (targetLevel === 'low') {
      ...
      speed = parseFloat(clamp(dist / Math.max(0.1, duration * 0.95), 1.0, 8.0).toFixed(2));
      ...
      if (!deadline) {
        ...
        const rawSpeed = dist / Math.max(0.1, deadline.t * duration * 0.95);
        speed = parseFloat(clamp(rawSpeed, 1.0, 8.0).toFixed(2));
      }
    }

    if (speed !== null && shot.players[defenderId]) {
